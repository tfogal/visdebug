...
func go_graph(ccfg *C.struct_node, nnodes uint) map[uintptr]*Node {
	...
	for i := range gocfg {
		...
		for e := range goedgelist {
			...
			if addr != uintptr(goedgelist[e].from) {
				panic("invalid previous addr")
			}
		}
	}
...
func loop_headers_helper(node *Node, seen map[uintptr]bool, elist []biEdge) {
	...
	if in == 2 && out == 2 {
		...
		ndom := 0
		if ins[0].To.Addr != node.Addr {
			panic("in_edges is broken")
		}
		if ins[1].To.Addr != node.Addr {
			panic("in_edges is broken")
		}
...
func addr_to_node(address uintptr, elist []biEdge) *Node {
	for _, e := range elist {
		if e.From.Addr == address {
			return e.From
		}
		if e.To.Addr == address {
			return e.To
		}
	}
	return nil
...
func dominance_helper(chld *Node, parent *Node) {
	...
	if chld.dominators.Len() == 0 { // first time we're here, initialize
		...
		if isect := intersection(chld.dominators, incoming); sameset(chld.dominators, isect) {
