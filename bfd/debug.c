...
static bool
dbgchannel_enabled(const struct tuvokdbgchannel* chn, enum DbgChanClass c)
{
...
static const char*
color(const enum DbgChanClass cls)
{
  if(!color_enabled) { return ""; }
  switch(cls) {
    case DbgTrace: return C_WHITE;
    case DbgWarn: return C_YELLOW;
    case DbgErr: return C_RED;
    case DbgFixme: return C_LBLUE;
  }
  assert(false);
  return C_NORM;
}
...
void
symb_dbg(enum DbgChanClass type, const struct tuvokdbgchannel* channel,
         const char* func, const char* format, ...)
{
  va_list args;
  va_start(args, format);
  if(dbgchannel_enabled(channel, type)) {
    const char* fixit = type==DbgFixme ? "-FIXME" : "";
 ...
 * every entry in 'enum DbgChanClass' to this. */
static enum DbgChanClass
name_class(const char* name)
{
  if(strncmp(name, "err", 3) == 0) { return DbgErr; }
  if(strncmp(name, "warn", 4) == 0) { return DbgWarn; }
  if(strncmp(name, "trace", 5) == 0) { return DbgTrace; }
  if(strncmp(name, "fixme", 5) == 0) { return DbgFixme; }
...
{
  _Static_assert(sizeof(enum DbgChanClass) <= sizeof(unsigned),
  ...
  for(const char* chan=opt; chan && chan!=(const char*)0x1;
    ...
    if(strncmp(chopts, ch->name, strlen(ch->name)) == 0) {
      ...
      for(char* olist=strchr(chopts, '=')+1;
        ...
        enum DbgChanClass cls = name_class(olist+1);
