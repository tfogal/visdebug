...
func MainSync(program string, inferior *ptrace.Tracee) error {
	...
	if symmain == nil {
		return fmt.Errorf("no 'main' function in symlist")
	}
...
func ihandle(inferior *ptrace.Tracee) (ievent, error) {
	...
	case status.Stopped():
		// There are many reasons a process might get Stopped: its children (i.e.
		// our grandchildren) died, the user ^Z/fg'd it, etc.  None of these are
		// things we'd care about, so just recurse and pretend they didn't happen.
...
func follow_jump(inferior *ptrace.Tracee, ixn x86asm.Inst,
	...
	if ismref {
		...
		case x86asm.RBP:
			...
			mr = uintptr(regs.Rbp)
		default:
			protection.Error("unhandled register base: %v\n", memref)
		}
...
func user_symbol(name string) bool {
	if strings.Contains(name, "calloc") {
		return true
	}
